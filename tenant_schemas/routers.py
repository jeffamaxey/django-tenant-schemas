...
class TenantSyncRouter(object):
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        ...
        if db_engine != 'tenant_schemas.postgresql_backend' and not issubclass(
            getattr(load_backend(db_engine), 'DatabaseWrapper'), TenantDbWrapper
        ):
            return None
        ...
        if connection.schema_name == get_public_schema_name():
            if app_label not in app_labels(settings.SHARED_APPS):
                return False
        elif app_label not in app_labels(settings.TENANT_APPS):
            return False
